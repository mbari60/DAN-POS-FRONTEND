...
const POSSystem = () => {
  ...
  const loadDailyReportPreview = async () => {
    setReportLoading(true);
    try {
      const reportData = await getDailyReportPreview(reportDate);
      setDailyReport(reportData);
      setShowDailyReport(true);
      toast.success("Daily report preview loaded successfully");
    } catch (err) {
      setError("Failed to load daily report: " + err.message);
      toast.error("Failed to load daily report: " + err.message);
    } finally {
  ...
  const handleSendDailyReport = async () => {
    ...
    try {
      const result = await sendDailyReport(reportDate, sendEmail || undefined);
      toast.success("Daily report sent successfully!");
      if (result.message) {
        setSuccess(result.message);
      }
    } catch (err) {
      setError("Failed to send daily report: " + err.message);
      toast.error("Failed to send daily report: " + err.message);
    } finally {
              ...
              <div className="mt-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                ...
                <div className="flex items-center gap-2 flex-1 max-w-md">
                  ...
                  <input
                    ...
                    placeholder="Optional: Email to send report (defaults to system email)"
                ...
                <div className="flex gap-2">
                  <button
                    onClick={loadDailyReportPreview}
                  ...
                  <button
                    ...
                    disabled={sendingReport || !dailyReport}
              ...
              {reportLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-xl">
                    <h1 className="text-2xl font-bold mb-2">Daily Sales Report</h1>
                    <p className="text-blue-100">Date: {dailyReport.preview_date || reportDate}</p>
                    <p className="text-blue-100">Generated on: {new Date().toLocaleDateString()}</p>
                  </div>

                  {/* Summary Section */}
                  {dailyReport.report_data?.summary && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="font-semibold text-gray-700 mb-2">Total Sales</h3>
                        <p className="text-3xl font-bold text-green-600">
                          {dailyReport.report_data.summary.formatted_total_sales || `KES ${dailyReport.report_data.summary.total_sales?.toLocaleString() || "0"}`}
                        </p>
                        <p className="text-sm text-gray-600 mt-1">{dailyReport.report_data.summary.total_invoices} invoices</p>
                      </div>
                      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="font-semibold text-gray-700 mb-2">Average Sale</h3>
                        <p className="text-3xl font-bold text-blue-600">
                          {dailyReport.report_data.summary.formatted_average_sale || `KES ${dailyReport.report_data.summary.average_sale?.toLocaleString() || "0"}`}
                        </p>
                      </div>
                      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="font-semibold text-gray-700 mb-2">Payments Received</h3>
                        <p className="text-3xl font-bold text-purple-600">
                          {dailyReport.report_data.summary.formatted_payments_received || `KES ${dailyReport.report_data.summary.total_payments_received?.toLocaleString() || "0"}`}
                        </p>
                      </div>
                      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="font-semibold text-gray-700 mb-2">Items Sold</h3>
                        <p className="text-3xl font-bold text-orange-600">
                          {dailyReport.report_data.metrics?.total_items_sold || 0}
                        </p>
                        <p className="text-sm text-gray-600 mt-1">{dailyReport.report_data.metrics?.unique_products_sold || 0} unique products</p>
                      </div>
                    </div>
                  )}

                  {/* Payment Breakdown */}
                  {dailyReport.report_data?.payment_breakdown && (
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <h3 className="text-xl font-semibold mb-4">Payment Breakdown</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        {Object.entries(dailyReport.report_data.payment_breakdown).map(([method, data]) => (
                          <div key={method} className="text-center p-4 bg-gray-50 rounded-lg">
                            <div className="capitalize font-semibold text-gray-800 mb-2">{method}</div>
                            <div className="text-2xl font-bold text-green-600 mb-1">{data.formatted}</div>
                  ...
                  {/* Top Products */}
                  {dailyReport.report_data?.top_products && dailyReport.report_data.top_products.length > 0 && (
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <h3 className="text-xl font-semibold mb-4">Top Selling Products</h3>
                      <div className="space-y-3">
                        {dailyReport.report_data.top_products.map((product, index) => (
                          <div key={index} className="flex justify-between items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                            <div>
                              ...
                              <span className="text-sm text-gray-600 ml-2">(Qty: {product.quantity})</span>
                            </div>
                  ...
                  {/* Low Stock Items */}
                  {dailyReport.report_data?.low_stock_items && dailyReport.report_data.low_stock_items.length > 0 && (
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <h3 className="text-xl font-semibold mb-4">Low Stock Alert</h3>
                      <div className="space-y-3">
                        {dailyReport.report_data.low_stock_items.map((item, index) => (
                          <div key={index} className={`p-3 border rounded-lg ${
                            ...
                            <div className="text-sm text-gray-600 mt-1">
                              Stock: {item.current_stock} | Reorder Level: {item.reorder_level} | Store: {item.store}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* Metrics */}
                  {dailyReport.report_data?.metrics && (
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                      <h3 className="text-xl font-semibold mb-4">Key Metrics</h3>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 bg-blue-50 rounded-lg">
                          <div className="text-2xl font-bold text-blue-600">{dailyReport.report_data.metrics.total_items_sold}</div>
                          <div className="text-sm text-gray-600">Items Sold</div>
                        </div>
                        <div className="text-center p-4 bg-green-50 rounded-lg">
                          <div className="text-2xl font-bold text-green-600">{dailyReport.report_data.metrics.unique_products_sold}</div>
                          <div className="text-sm text-gray-600">Unique Products</div>
                        </div>
                        <div className="text-center p-4 bg-red-50 rounded-lg">
                          <div className="text-2xl font-bold text-red-600">{dailyReport.report_data.metrics.critical_stock_items}</div>
                          <div className="text-sm text-gray-600">Critical Stock</div>
                        </div>
                        <div className="text-center p-4 bg-orange-50 rounded-lg">
                          <div className="text-2xl font-bold text-orange-600">{dailyReport.report_data.metrics.high_stock_items}</div>
                          <div className="text-sm text-gray-600">High Stock Alert</div>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">Click "Preview Report" to generate daily report</p>
                </div>
              )}
            </div>
          </div>
